                            ...
                            <div class="col-lg-6">
                                <div class="form-group">
                                    ...
                                    <div
                                        ...
                                        id="kt_image_5"
                                        style="background-image: url(storage/images/vehicle-photo-default.png)"
                                    >
                            ...
                            <div class="col-lg-6">
                                <div class="form-group">
                                    ...
                                    <input
                                        ...
                                        v-model="formFields.templateNumber"
                                    />
                                </div>
                                <div class="form-group">
                                    ...
                                    <input
                                        type="number"
                                        class="form-control required-field"
                                        name="vehicle_capacityNumber"
                                        placeholder="Enter capacity number"
                                        v-model="formFields.capacityNumber"
                                    />
                                </div>
                                <div class="form-group">
                                    <label>Remarks:</label>
...
export default {
    ...
    methods: {
        ...
        editEntry(id) {
            ...
            $(() => {
                axios
                    .get(BASE_URL + "/transportation/vehicle/" + id)
                    .then(response => {
                        vm.formFields.id = response.data.vehicles[0].id;
                        vm.formFields.pictureName =
                            response.data.vehicles[0].image;
                        vm.formFields.name = response.data.vehicles[0].name;
                        vm.formFields.description =
                            response.data.vehicles[0].description;
                        vm.formFields.capacityNumber =
                            response.data.vehicles[0].capacity;
                        vm.formFields.templateNumber =
                            response.data.vehicles[0].plate_no;
                        vm.formFields.status = response.data.vehicles[0].status;
                        vm.formFields.remarks =
                            response.data.vehicles[0].remarks;

                        let img = response.data.vehicles[0].image
                            ? BASE_URL +
                              "/storage/images/" +
                              response.data.vehicles[0].image
                            : BASE_URL +
                              "/storage/images/vehicle-photo-default.png";
                        $("#kt_image_5").css(
                            "background-image",
                            "url(" + img + ")"
                        );
                ...
                .catch(error => {
                        ...
                        if (`${key}` == "status_radio") {
                            if (
                                ...
                                $(".checkbox-inline").after(
                                    '<div class="invalid-feedback invalid-feedback d-block">' +
                                        `${value}` +
                                        "</div>"
                                );
                            }
                        } else if (
                            $('[name="vehicle_' + `${key}` + '"]').next()
                                .length == 0 ||
                            $('[name="vehicle_' + `${key}` + '"]')
                                .next()
                                .attr("class")
                            ...
                            $('[name="vehicle_' + `${key}` + '"]').addClass(
                                "is-invalid"
                            );
                            $('[name="vehicle_' + `${key}` + '"]').after(
                    ...
                    for (let i = 0; i < this.names.length; i++) {
                        if (this.names[i] == "status_radio") {
                            ...
                            keys.indexOf("" + this.names[i] + "") == -1
                        ) {
                            $(
                                '[name="vehicle_' + this.names[i] + '"]'
        ...
        deleteEntry(id) {
            Swal.fire({
                ...
                if (result.value) {
                    axios
                        .delete(BASE_URL + "/transportation/vehicle/" + id)
                        .then(response => {
                            Swal.fire(
                                "Deleted!",
                ...
                table.DataTable({
                    ...
                    columnDefs: [
                        ...
                        },
                        {
                            ...
                            render: data => {
                                var img_path = data
                                    ? BASE_URL + "/storage/images/" + data
                                    : BASE_URL +
                                      "/storage/images/vehicle-photo-default.png";
                                return (
                                    '<a class="vehicle-img-viewer" href="' +
                                    img_path +
                                    '"><img class="img-fluid img-thumbnail vehicle-img" src="' +
                                    img_path +
                                    '"></a>'
                                );
                            }
