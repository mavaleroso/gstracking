<template>
    <div id="serviceProvider-page">
        ...
        <div v-else class="card card-custom gutter-b animate__animated animate__fadeIn" >
            ...
            <div class="card-body">
                <!--begin: Datatable-->
                <table class="table table-separate table-head-custom table-checkable" id="serviceProvider-tbl">
                    <thead>
                        <tr>
                            <th>ID</th>
                            <th>Type</th>
                    ...
                    for (const [key, value] of Object.entries(data)) {
                        ...
                        if(`${key}` == 'type'){
                            if ($('#kt_select_svc_type').next().next().length == 0) {
                                $('#kt_select_svc_type').next().after('<div class="invalid-feedback d-block">'+`${value}`+'</div>');
                            }
                        } else {
                       
                            if ($('[name="svc_'+`${key}`+'"]').next().length == 0 || $('[name="svc_'+`${key}`+'"]').next().attr('class').search('invalid-feedback') == -1) {
                                $('[name="svc_'+`${key}`+'"]').addClass('is-invalid');
                                $('[name="svc_'+`${key}`+'"]').after('<div class="invalid-feedback">'+`${value}`+'</div>');
                            }
                        }
                    }
                    for (let i = 0; i < this.names.length; i++) {
                        if (this.names[i] == 'type') {
                            ...
                            if (keys.indexOf(''+this.names[i]+'') == -1) {
                                $('[name="svc_'+this.names[i]+'"]').removeClass('is-invalid');
                                $('[name="svc_'+this.names[i]+'"]').next('.invalid-feedback').remove();
                            }
                        }
                    }
