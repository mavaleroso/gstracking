    ...
    <modal :status="status">
      <template v-slot:header>
        ...
        <h5 class="modal-title">
          ...
          <span class="d-block text-muted font-size-sm">Reference Code</span>
        </h5>
        <h3 class="modal-date">
          <span class="m-date">{{ dateTimeEng }}</span>
          <span class="d-block text-muted font-size-sm">Date Created</span>
        </h3>
        <button
          v-if="status == 1"
          @click="edit"
                    ...
                    <tr v-for="(pax, index) in passengers" :key="index">
                      ...
                      <td>
                        <select
                          ...
                          :id="'passenger-select-' + paxIndex(index)"
                          :name="'pax_name_' + paxIndex(index)"
                          :disabled="!request_edit ? true : false"
                          :value="pax.name"
                        >
                          <option label="Label"></option>
                          <option
                            v-for="(result, index) in employee_results"
                            :key="index"
                            :value="
                              result.first_name +
                              ' ' +
                      ...
                      <td>
                        <input
                          :name="'pax_des_' + paxIndex(index)"
                          :id="'pax_des_' + paxIndex(index)"
                          class="data-entry form-control"
                          type="text"
                          disabled="disabled"
                      ...
                      <td>
                        ...
                        <select
                          :name="'pax_gen_' + paxIndex(index)"
                          :id="'pax_gen_' + paxIndex(index)"
                          class="data-entry form-control"
                          disabled="disabled"
                          :value="pax.gender"
                        >
          ...
          <div class="card-body row">
              ...
              <div class="col-lg-4">
                <div v-if="vehicle_type == 4" class="form-group">
                  ...
                  >
                    <option label="Label"></option>
                    <option
                      v-for="po in procurements.filter((i) => i.type == 1)"
                      :key="po.id"
                      :value="po.id"
                    >
                      {{ po.po_no }} - ₱
            ...
            <div v-if="vehicle_type == 1" class="col-lg-12 row">
              <label class="h5 col-lg-12">{{
                vehiclemodes.filter((i) => i.id == vehicle_type)[0].name
              }}</label>
        ...
        .catch((error) => {
          ...
          for (let i = 0; i < this.names.length; i++) {
            if (keys.indexOf("" + this.names[i] + "") == -1) {
              $('input[name="' + this.names[i] + '"]').removeClass(
                "is-invalid"
              );
              $('[name="' + this.names[i] + '"]')
        ...
        .catch((error) => {
          ...
          for (let i = 0; i < this.hiredNames.length; i++) {
            if (this.hiredNames[i] == "travel_po") {
              ...
              if (keys.indexOf("" + this.hiredNames[i] + "") == -1) {
                $('[name="' + this.hiredNames[i] + '"]').removeClass(
                  "is-invalid"
                );
      ...
      axios
        ...
        .catch((error) => {
          ...
          for (const [key, value] of Object.entries(data)) {
            ...
            values.push(`${value}`);
            if (
              $('[name="declined_' + `${key}` + '"]').next().length == 0 ||
              $('[name="remarks_' + `${key}` + '"]')
                .next()
                .attr("class")
                .search("invalid-feedback") == -1
